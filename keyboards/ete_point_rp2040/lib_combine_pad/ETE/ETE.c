...
void keyboard_post_init_kb(void) {
    if (eeconfig_is_enabled()) {
        ...
        uint8_t cpi  = ete_conf_get_cpi(raw);
        uint8_t sdiv = ete_conf_get_sdiv(raw);

        ETE_set_cpi(cpi == 0 ? CPI_DEFAULT : cpi);
        ETE_set_scroll_div(sdiv == 0 ? ETE_SCROLL_DIV_DEFAULT : sdiv);
    }
